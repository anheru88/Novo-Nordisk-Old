...
var vueF = new Vue({
    ...
    methods: {
        ...
        getProductsClientQuota: function () {
            this.fechaini = $("#quota_date_ini").val();
            this.fechafin = $("#quota_date_end").val();
            this.client = $('#client').val();
            //console.log(this.client);
            var urlProduct = '../getProductsClientQuota';
            if (this.client == "" || this.fechaini == "" || this.fechafin == "") {
                toastr.options = {
                    "closeButton": false,
                    "progressBar": false,
                    "newestOnTop": false
                };
                toastr.error('Debe seleccionar todos los campos para realizar la carga').css("width", "auto")
                return;
            }

            this.client_name = $('#client :selected').text();
            this.loading = true;
            axios.post(urlProduct, {
                    ...
                    if (response.data == "No data") {
                        ...
                        this.inputProducts = [];
                        this.listProducts = [];
                        $('.clientes-select').val('').trigger('change');
                        this.client = "";
                        $("#quota_date_ini").val("");
                        $("#quota_date_end").val("");
                        $("#days").val("");
                        ...
                        for (let index = 0; index < this.productsArray.length; index++) {
                            ...
                            this.listQuotation.push(this.productsArray[index].id_quotation);
                        }
        ...
        getProductsClientDesc: function () {
            ...
            this.prodQuotation      = this.listQuotation[this.prodPosition - 1]
            ...
            axios.post(urlProduct, {
                    ...
                    idQuotation:        this.prodQuotation
            }).then(response => {
        ...
        negociacionAsistida: function (name) {
            ...
            for(var i=0; i<checkboxes.length; i++){
                if(checkboxes[i].checked){
                    this.selectedProducts.push( this.quotaProductsList[parseInt(checkboxes[i].value)]);
                    this.selectedQuotas.push(this.listQuotation[parseInt(checkboxes[i].value)]);
                }
            }
            ...
            axios.post(urlQuery, {
                ...
                quotationIds: this.selectedQuotas,
            }).then(response => {
        ...
        calcDiscount: function () {
            //console.log(this.prodQuotation);
        ...
        negociacionAsistidaxConcepto:function(name){
            ...
            axios.post(urlQuery, {
                ...
                pay_discount:       this.pay_discount,
                quotaProductsList:  this.selectedProducts, // lista de los productos cotizados por el cliente
                quotationIds:       this.selectedQuotas, // lista de id de cotizaciones de productos seleccionados
                quotedPrice:        this.quotedPrice,
            }).then(response => {
                this.productsAsistida = response.data;
        ...
        sendNegotiation: function (event) {
           ...
           // console.log(this.inputProducts);
            this.inputProducts.forEach(element => {
                if(element['warning'] < 2){
                    validProducs = 1;
                }
            });
            if (this.inputProducts <= 0 || validProducs == 0) {
                Swal.fire(
                    'Alerta',
                    'Debe añadir un descuento negociado valido',
                    'warning',
                );
            }else{
                this.saveForm();
            ...
            if(prod){
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $( ".productsNeg" ).prop( "checked", false );
                        $( "#remove-all" ).prop( "checked", false );
                        this.removeAllProductsAsistida('productsNeg');
                ...
                products: this.inputProducts
                   ...
                   // console.log(response.data);
                    if (response.data != "ERROR") {
                        this.idNegotiation = response.data;
                        $('#idNegotiation').val(this.idNegotiation);
